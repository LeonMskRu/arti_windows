...
def is_bad_external_ip_peer(ip, port, external_ip):
    """Check if peer is external IP with port 6881 (bad) or different port (working)"""
    if ip == external_ip:
        if port == 6881:
            return True  # Bad tracker - returns our external IP with port 6881
        else:
            return False  # Working tracker - returns our external IP with different port
    return False  # Not our external IP
...
# -----------------------
# Save files with updated logic for WORK and GOOD
# -----------------------
def save_additional_files(results, base_path, common_ips, external_ip):
    ...
    NEW: Check for external IP with port 6881 (bad) vs external IP with different port (working)
    """
    work_trackers = []
    good_trackers = []
    ...
    for r in results:
        ...
        # Check for bad external IP peers (external IP + port 6881)
        ...
        # Detect any bad IP present (now includes blocklist)
        any_bad = any(is_bad_ip(ip) for ip, _ in all_peers)

        # WORK condition: status OK AND not echo-only AND no bad IPs AND no bad external IP (port 6881)
        if not all_echo and not any_bad and not has_bad_external_ip:
...
def save_detailed_ip_files(results, base_path, common_ips, external_ip):
    ...
    for result in results:
        if result["status"] == "OK" and result["all_peer_ips"]:
            ...
            # Check for bad external IP (port 6881)
            ...
            all_ip_data.append(f"# Suspicious: {suspicious}, Has external IP: {has_external_ip}, Has bad external IP (6881): {has_bad_external_ip}")
            ...
            for ip, port in result["all_peer_ips"]:
                ...
                all_ip_data.append(f"{tracker_name} {peer_type} {ip}:{port} # {ip_type}{is_external}{is_bad_external}")
            all_ip_data.append("")
    ...
    for result in results:
        if result["status"] == "OK" and result["good_peer_ips"]:
            ...
            good_ip_data.append(f"# Has external IP: {has_external_ip}, Has bad external IP (6881): {has_bad_external_ip}")
            
            for ip, port in result["good_peer_ips"]:
                peer_type = "seed" if result["seeders"] > 0 else "peer"
                is_external = " [EXTERNAL]" if ip == external_ip else ""
                is_bad_external = " [BAD_EXTERNAL_6881]" if is_bad_external_ip_peer(ip, port, external_ip) else ""
                good_ip_data.append(f"{tracker_name} {peer_type} {ip}:{port}{is_external}{is_bad_external}")
            good_ip_data.append("")
...
def save_statistics(results, base_path, external_ip, common_ips):
    ...
    # NEW criteria with external IP port checking
    work_new = 0
    good_new = 0
    ...
    stats.append(f"Work trackers (non-echo, no-bad-ip, no-bad-external-6881): {work_new}")
