            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as no_proxy_session:
                async with no_proxy_session.get(service, timeout=timeout) as resp:
                    if resp.status == 200:
                        ...
                        # Extract IPv6 address using regex
                        ...
                        for ip_match in ip_matches:
                            try:
                                ...
                                if ip.version == 6:
                                    ip_str = str(ip)
                                    # Filter only IPv6 addresses with our prefix and create /56 network
                                    if ip_str.startswith('2a00:62c0:'):
                                        # Create /56 network from the IPv6 address
                                        network_str = ip_str[:14] + "::/64"  # 2a00:62c0:xxxx:yyyy::/56
                                        try:
...
def is_bad_external_ip_peer(ip, port, external_ipv4, external_ipv6_networks):
    ...
    # Check IPv6 addresses in our /56 networks
    try:
        ip_obj = ipaddress.ip_address(ip)
        if ip_obj.version == 6:
...
def save_statistics(results, base_path, external_ipv4, external_ipv6_networks, common_ips):
    ...
    stats.append(f"External IPv6 networks (/56): {len(external_ipv6_networks)}")
    for network in external_ipv6_networks:
...
async def main_async(args):
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=session_timeout) as session:
        ...
        print(f"Using external IPv6 networks (/56): {len(external_ipv6_networks)}")
        for network in external_ipv6_networks:
            print(f"  - {network}")
    ...
    print(f"External IPv6 networks (/56): {len(external_ipv6_networks)}")
