...
def save_additional_files(results, base_path, common_ips, external_ipv4, external_ipv6_networks, bad_flag=False):
    ...
    - With --bad:
        WORK и GOOD: включают трекеры, которые отдают наш внешний IP (IPv4 или IPv6) даже с портом 6881,
        но проверяют отсутствие других "неправильных" IP (bogon/private/blocklisted)
    """
    work_trackers = []
    good_trackers = []
    ...
    for r in results:
        ...
        if bad_flag:
            # С опцией --bad: WORK и GOOD включают трекеры с нашим внешним IP (даже с портом 6881)
            # но исключают трекеры с другими плохими IP
            
            # WORK условие с --bad: нет других плохих IP (наш external_ip:6881 разрешен)
            if not has_other_bad_ips:
                work_trackers.append(r["tracker"])

            # GOOD условие с --bad: есть хотя бы один не-bogon IP (наш external_ip разрешен)
