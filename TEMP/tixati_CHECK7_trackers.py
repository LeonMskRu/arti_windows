...
async def get_external_ips(session):
            ...
            async with aiohttp.ClientSession(connector=connector, timeout=timeout) as no_proxy_session:
                async with no_proxy_session.get(service, timeout=timeout) as resp:
                    if resp.status == 200:
                        ...
                        if ip.version == 6:
                            ip_str = str(ip)
                            # Filter only IPv6 addresses with our prefix
                            if ip_str.startswith('2a00:62c0:'):
                                if ip_str not in external_ipv6_list:
                                    external_ipv6_list.append(ip_str)
    ...
    return external_ipv4, external_ipv6_list
...
# Suspicious check
# -----------------------
def is_suspicious_tracker(result, common_ips, external_ipv4, external_ipv6_list):
    if result["status"] != "OK":
        return True
...
# -----------------------
def is_bad_external_ip_peer(ip, port, external_ipv4, external_ipv6_list):
    """Check if peer is external IP with port 6881 (bad)"""
    if ip == external_ipv4 and port == 6881:
        return True
    
    # Check IPv6 addresses with our prefix
    for external_ipv6 in external_ipv6_list:
        if ip == external_ipv6 and port == 6881:
            return True
...
# -----------------------
def is_good_external_ip_peer(ip, port, external_ipv4, external_ipv6_list):
...
# -----------------------
def is_our_external_ip(ip, external_ipv4, external_ipv6_list):
...
# -----------------------
def save_additional_files(results, base_path, common_ips, external_ipv4, external_ipv6_list):
    """
    NEW LOGIC:
    - WORK: 
        1. статус OK
    ...
    for r in results:
        ...
        has_bad_external_ip = any(is_bad_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) for ip, port in all_peers)
        ...
        for ip, port in all_peers:
            if (not is_bad_ip(ip)) and not is_our_external_ip(ip, external_ipv4, external_ipv6_list):
...
# -----------------------
def save_detailed_ip_files(results, base_path, common_ips, external_ipv4, external_ipv6_list):
    all_ip_data = []
    for result in results:
        if result["status"] == "OK" and result["all_peer_ips"]:
            tracker_name = urlparse(result["tracker"]).netloc.replace(':', '_')
            suspicious = is_suspicious_tracker(result, common_ips, external_ipv4, external_ipv6_list)
            has_bad_external_ip = any(is_bad_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) for ip, port in result["all_peer_ips"])
            has_good_external_ip = any(is_good_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) for ip, port in result["all_peer_ips"])
            ...
            for ip, port in result["all_peer_ips"]:
                ...
                is_bad_external = " [BAD_EXTERNAL_6881]" if is_bad_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) else ""
                is_good_external = " [GOOD_EXTERNAL]" if is_good_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) else ""
    ...
    for result in results:
        if result["status"] == "OK" and result["good_peer_ips"]:
            ...
            if is_suspicious_tracker(result, common_ips, external_ipv4, external_ipv6_list):
                continue
            tracker_name = urlparse(result["tracker"]).netloc.replace(':', '_')
            has_bad_external_ip = any(is_bad_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) for ip, port in result["all_peer_ips"])
            has_good_external_ip = any(is_good_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) for ip, port in result["all_peer_ips"])
...
# Save statistics
# -----------------------
def save_statistics(results, base_path, external_ipv4, external_ipv6_list, common_ips):
    stats = []
    ...
    for r in results:
        ...
        has_bad_external_ip = any(is_bad_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) for ip, port in peers)
        has_other_bad_ips = any(is_bad_ip(ip) for ip, port in peers)
        has_good_external_ip = any(is_good_external_ip_peer(ip, port, external_ipv4, external_ipv6_list) for ip, port in peers)
        ...
        if any((not is_bad_ip(ip)) and not is_our_external_ip(ip, external_ipv4, external_ipv6_list) for ip, _ in peers):
            good_new += 1
    ...
    for r in results:
        if r["status"] == "OK":
            for ip, port in r["all_peer_ips"]:
                if is_our_external_ip(ip, external_ipv4, external_ipv6_list):
                    if port == 6881:
    ...
    stats.append(f"External IPv6 addresses: {len(external_ipv6_list)}")
    for ipv6 in external_ipv6_list:
        stats.append(f"  - {ipv6}")
    ...
    suspicious_trackers = [r for r in results if r["status"] == "OK" and is_suspicious_tracker(r, common_ips, external_ipv4, external_ipv6_list)]
...
async def main_async(args):
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=session_timeout) as session:
        ...
        external_ipv4, external_ipv6_list = await get_external_ips(session)
        print(f"Using external IPv4: {external_ipv4}")
        print(f"Using external IPv6 addresses: {len(external_ipv6_list)}")
        for ipv6 in external_ipv6_list:
            print(f"  - {ipv6}")
    ...
    save_additional_files(results, base_path, common_ips, external_ipv4, external_ipv6_list)
    save_detailed_ip_files(results, base_path, common_ips, external_ipv4, external_ipv6_list)
    save_statistics(results, base_path, external_ipv4, external_ipv6_list, common_ips)
    ...
    print(f"External IPv6 addresses: {len(external_ipv6_list)}")
